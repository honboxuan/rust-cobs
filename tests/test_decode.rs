...
macro_rules! test_decode {
            ($name:ident, $( ( $input:expr, $output:expr )$(,)* )+) => {
                ...
                fn $name() {
                    ...
                    $( assert_eq!(cobs.decode($input), $output); )+
                }
            };
        }

// Test single, complete packets
test_decode!(test_00, (&[0x01, 0x00], Some(vec![vec![]])));
test_decode!(test_01, (&[0x01, 0x01, 0x00], Some(vec![vec![0x00]])));
test_decode!(
    test_02,
    (&[0x01, 0x01, 0x01, 0x00], Some(vec![vec![0x00, 0x00]]))
);
test_decode!(
    test_03,
    (
        &[0x01, 0x02, 0x11, 0x01, 0x00],
        Some(vec![vec![0x00, 0x11, 0x00]])
    )
);
test_decode!(
    test_04,
    (
        ...
        Some(vec![vec![0x11, 0x22, 0x00, 0x33]])
    )
);
test_decode!(
    test_05,
    (
        ...
        Some(vec![vec![0x11, 0x22, 0x33, 0x44]])
    )
);
test_decode!(
    test_06,
    (
        ...
        Some(vec![vec![0x11, 0x00, 0x00, 0x00]])
    )
);
...
test_decode!(
    ...
    (
        ...
        Some(vec![vec![0x00], vec![0x00, 0x00]])
    )
);
test_decode!(
    test_08,
    (
        &[0x01, 0x02, 0x11, 0x01, 0x00, 0x03, 0x11, 0x22, 0x02, 0x33, 0x00],
        Some(vec![vec![0x00, 0x11, 0x00], vec![0x11, 0x22, 0x00, 0x33]])
    )
);
test_decode!(
    test_09,
    (
        &[0x05, 0x11, 0x22, 0x33, 0x44, 0x00, 0x02, 0x11, 0x01, 0x01, 0x01, 0x00],
        Some(vec![
...
// Test consecutive, single, complete packets
test_decode!(
    test_10,
    (
        &[0x02, 0x11, 0x01, 0x01, 0x01, 0x00],
...
// Test incomplete packet, consecutive calls
test_decode!(
    test_11,
    (&[0x01, 0x02, 0x11, 0x01], None),
    (&[0x00], Some(vec![vec![0x00, 0x11, 0x00]]))
);

// Test incomplete packet and multi-packet
test_decode!(
    test_12,
    (&[0x01, 0x02, 0x11, 0x01], None),
    (
        &[0x00, 0x01, 0x01, 0x00, 0x01, 0x01, 0x01, 0x00],
        Some(vec![vec![0x00, 0x11, 0x00], vec![0x00], vec![0x00, 0x00]])
    )
);

// Test large packet (> 254 bytes)
test_decode!(
    test_13,
    (
        &[
            ...
            0xFB, 0xFC, 0xFD, 0xFE, 0x02, 0xFF, 0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
            ...
            0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0x02, 0xFF, 0x00
        ],
        Some(vec![vec![
...
// Test unexpected zero in packet
test_decode!(test_14, (&[0x05, 0x11, 0x22, 0x33, 0x00], None));
test_decode!(test_15, (&[0x00, 0x11, 0x22, 0x33, 0x00], None));

// Test unexpected zero in incomplete packet
test_decode!(test_16, (&[0x05, 0x11, 0x22], None), (&[0x33, 0x00], None));

// Test unexpected zero in packet, followed by complete packet
test_decode!(
    test_17,
    (
        &[0x05, 0x11, 0x22, 0x33, 0x00, 0x05, 0x11, 0x22, 0x33, 0x44, 0x00],
        Some(vec![vec![0x11, 0x22, 0x33, 0x44]])
    )
);
